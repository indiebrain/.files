;;; indiebrain-vc.el --- Extensions to vc.el for my dotemacs -*- lexical-binding: t -*-

;; Copyright (C) 2012-2021  Aaron Kuehler

;; Author: Aaron Kuehler <aaron.kuehler+public@gmail.comn>
...
;;; Commentary:
;;
;; This covers my vc.el extensions, which mostly concern Git. For use in
...
(defgroup indiebrain-vc ()
  "Extensions for vc.el and related libraries."
  :group 'project)

(defcustom indiebrain-vc-log-limit 100
  "Limit commits in `indiebrain-vc-custom-log' and others."
  :type 'integer
  :group 'indiebrain-vc)

(defcustom indiebrain-vc-log-bulk-action-limit 50
  "Limit for `indiebrain-vc-log-view-toggle-entry-all'.
This is to ensure that performance does not take a hit.  The
default value is conservative."
  :type 'integer
  :group 'indiebrain-vc)

(defcustom indiebrain-vc-git-log-edit-show-commits nil
  "Show recent commits in Git Log Edit comments."
  :type 'boolean
  :group 'indiebrain-vc)

(defcustom indiebrain-vc-git-log-edit-show-commit-count 10
  "Commit number for `indiebrain-vc-git-log-edit-show-commits'."
  :type 'integer
  :group 'indiebrain-vc)

(defcustom indiebrain-vc-shell-output "*indiebrain-vc-output*"
  "Name of buffer for VC-related shell output."
  :type 'string
  :group 'indiebrain-vc)

(defcustom indiebrain-vc-patch-output-dirs (list "~/" "~/Desktop/")
  "List of directories to save `indiebrain-vc-patch-dwim' output."
  :type 'list
  :group 'indiebrain-vc)

;;;; Commands and helper functions

(defun indiebrain-vc--current-project ()
  "Return root directory of current project."
  (or (vc-root-dir)
      (locate-dominating-file "." ".git")))

;;;###autoload
(defun indiebrain-vc-project-or-dir (&optional arg)
  "Run `vc-dir' for the current project root.
With optional prefix ARG (\\[universal-argument]), use the
`default-directory' instead."
  (interactive "P")
  (let* ((root (indiebrain-vc--current-project))
         (dir (if arg default-directory root)))
    (vc-dir dir)))

(defun indiebrain-vc--log-edit-files-prompt ()
  "Helper completion for `indiebrain-vc-extract-file-name'."
  (let ((files (log-edit-files)))
    (completing-read
     "Derive shortname from: " files nil nil)))

;;;###autoload
(defun indiebrain-vc-git-log-edit-extract-file-name ()
...
existing item or input an arbitrary string of characters."
  ...
  (let* ((files (log-edit-files))
         (file (if (> (length files) 1)
                   (indiebrain-vc--log-edit-files-prompt)
                 (car files)))
...
(defvar indiebrain-vc--log-insert-num-hist '()
  "History for `indiebrain-vc-git-log-insert-commits'.")

(declare-function project-prompt-project-dir "project")

;;;###autoload
(defun indiebrain-vc-git-log-insert-commits (&optional arg)
...
for a known project."
  ...
  (let* ((dir (when arg (project-prompt-project-dir)))
         (default-directory (or dir
                                (indiebrain-vc--current-project)
                                (cdr (project-current t))))
         (number (number-to-string
                 (read-number "Insert N commits from HEAD: " 5
                              'indiebrain-vc--log-insert-num-hist))))
    (insert
     (with-temp-buffer
       (apply 'vc-git-command t nil nil
              (list "log" "--pretty=format:%h  %cs  %s" "-n" number "--"))
       (buffer-string)))
    (add-to-history 'indiebrain-vc--log-insert-num-hist number)))

(autoload 'log-view-current-entry "log-view")
(autoload 'dired-get-marked-files "dired")

(defun indiebrain-vc--commit-num ()
  "Determime whether `indiebrain-vc-log-limit' is a positive integer."
  (let ((num indiebrain-vc-log-limit))
    (if (and (integerp num)
             (> num 0))
        num
...
;;;###autoload
(defun indiebrain-vc-custom-log (&optional arg)
...
The default limit is controlled by the `indiebrain-vc-log-limit'
variable.
         ...
         (set (cond                     ; REVIEW: this is confusing
               ...
               (t
                (completing-read-multiple
                 ...
                 #'indiebrain-common-crm-exclude-selected-p t))))
...
;;;###autoload
(defun indiebrain-vc-log-view-toggle-entry-all ()
  ...
  (let ((oldlines (count-lines (point-min) (point-max)))
        ...
        (commits (count-matches (nth 1 vc-git-root-log-format)
                                (point-min) (point-max)))
        (limit indiebrain-vc-log-bulk-action-limit))
    (cond
     ((<= commits limit)
      (save-excursion
        (goto-char (point-max))
...
;;;###autoload
(defun indiebrain-vc-log-kill-hash ()
...
(defvar indiebrain-vc--commit-hist '()
  "Minibuffer history for commit logs.")

(defvar indiebrain-vc--patch-output-hist '()
  "Minibuffer history for `indiebrain-vc-patch-dwim' output.")

(defun indiebrain-vc--log-commit-hash (fn)
  "Extract commit hash from FN.
FN is assumed to be something like `indiebrain-vc--log-commit-prompt'."
  (string-match "· \\([a-z0-9]*\\) ·" fn)
  (match-string-no-properties 1 fn))

(defun indiebrain-vc--log-commit-prompt (&optional prompt limit)
  "Select git log commit with completion.

Optional PROMPT pertains to the minibuffer's input field.  While
optional LIMIT will apply `indiebrain-vc-log-limit' as a constraint,
instead of producing a complete log."
  (let ((text (or prompt "Select a commit: "))
        (vc (indiebrain-vc--current-project))
        (num (cond
              ((integerp limit)
               (format "%d" limit))
              (limit
               (format "%d" (indiebrain-vc--commit-num)))
              (t
               (format "%d" -1)))))
    (if vc
        (completing-read
         ...
         nil t nil 'indiebrain-vc--commit-hist)
      (error "'%s' is not under version control" default-directory))))

;;;###autoload
(defun indiebrain-vc-git-patch-dwim (&optional arg)
...
is always confined to `indiebrain-vc-log-limit'."
  (interactive "P")
  (let* ((vc-dir (or (indiebrain-vc--current-project)
                     default-directory))
         (dirs (append (list vc-dir) indiebrain-vc-patch-output-dirs))
         (out-dir
          (completing-read
           "Output directory: "
           (indiebrain-common-completion-table 'file dirs)
           nil t nil 'indiebrain-vc--patch-output-hist))
         (buf (get-buffer-create indiebrain-vc-shell-output)))
    (cond
     ...
     (arg
      (let ((base (indiebrain-vc--log-commit-hash
                   (indiebrain-vc--log-commit-prompt
     ...
     (t
      (let* ((commit-at-point (when (derived-mode-p 'log-view-mode)
        ...
        (message "Prepared patch for `%s' and sent it to %s"
                 ...
                 (propertize out-dir 'face 'success))
        (add-to-history 'indiebrain-vc--commit-hist commit)))
     (add-to-history 'indiebrain-vc--patch-output-hist out-dir))))

;;;###autoload
(defun indiebrain-vc-git-show (&optional limit)
...
accept it directly.  In the absence of LIMIT, `indiebrain-vc-log-limit'
will be used instead."
  ...
  (let* ((num (cond
               ...
               (t
                t)))
         (commit (indiebrain-vc--log-commit-hash
                  (indiebrain-vc--log-commit-prompt "Commit to git-show: " num)))
         (buf-name indiebrain-vc-shell-output)
...
;;;###autoload
(defun indiebrain-vc-git-grep (regexp)
...
the original command for its other features."
  (interactive
   (list (read-regexp "git-grep for PATTERN: "
                      nil 'grep-history)))
  (vc-git-grep regexp "*" (indiebrain-vc--current-project)))

(autoload 'vc-git-region-history-mode "vc-git")

;;;###autoload
(defun indiebrain-vc-git-log-grep (pattern &optional diff)
...
argument, also show the corresponding diffs."
  (interactive
   (list (read-regexp "Run 'git log --grep' for PATTERN")
         current-prefix-arg))
   (let* ((buf-name indiebrain-vc-shell-output)
          (buf (get-buffer-create buf-name))
          (diffs (if diff "-p" ""))
...
(defun indiebrain-vc-git--file-rev (file &optional limit)
...
;;;###autoload
(defun indiebrain-vc-git-find-revision (&optional limit)
...
will be used instead."
  ...
  (let* ((num (cond
               ...
               (t
                t)))
         (rev (indiebrain-vc--log-commit-hash
               (indiebrain-vc-git--file-rev buffer-file-name num))))
    (switch-to-buffer-other-window
     (vc-find-revision buffer-file-name rev))
    (add-to-history 'indiebrain-vc--commit-hist rev)))

(autoload 'vc-annotate-mode "vc-annotate")
(autoload 'vc-annotate-display-select "vc-annotate")

;;;###autoload
(defun indiebrain-vc-git-blame-region-or-file (beg end)
  ...
  (let* ((buf-name indiebrain-vc-shell-output)
         ...
         (default-directory (indiebrain-vc--current-project))
         (resize-mini-windows nil))
    (shell-command
...
;;;###autoload
(defun indiebrain-vc-git-reset (&optional limit)
...
will be used instead."
  ...
  (let* ((num (cond
               ...
               (t
                t)))
         (commit (indiebrain-vc--log-commit-hash
                  (indiebrain-vc--log-commit-prompt "Run 'git reset --soft' on: " num)))
         (buf-name indiebrain-vc-shell-output)
         (buf (get-buffer-create buf-name)))
...
;;;###autoload
(defun indiebrain-vc-git-log-reset (&optional hard)
...
;;;###autoload
(defun indiebrain-vc-git-checkout-remote (remote)
...
(defun indiebrain-vc-git-expanded-log-entry (revision)
...
(defun indiebrain-vc-git-expand-function ()
  "Set `log-view-expanded-log-entry-function' for `vc-git'."
  (when (eq vc-log-view-type 'short)
    (setq-local log-view-expanded-log-entry-function
                #'indiebrain-vc-git-expanded-log-entry)))

(defvar indiebrain-vc-git-log-view-mode-hook nil
  "Hook that runs after `vc-git-log-view-mode'.")

(defun indiebrain-vc-git-log-view-add-hook (&rest _)
  "Run `indiebrain-vc-git-log-view-mode-hook'."
  (run-hooks 'indiebrain-vc-git-log-view-mode-hook))

(declare-function log-edit-add-field "log-edit")

(defun indiebrain-vc--format-git-comment (branch remote files &optional commits)
...
(defun indiebrain-vc-git-log-edit-comment (&optional no-headers)
...
headers 'Amend' and 'Summary'."
  (let* ((branch-name (process-lines "git" "branch" "--show-current"))
         ...
         (files (mapconcat (lambda (x)
                           ...
                           (log-edit-files)
                           "\n"))
         (commits (when (and indiebrain-vc-git-log-edit-show-commits
                             (ignore-errors (process-lines "git" "log" "-1")))
                    (mapconcat (lambda (x)
                                 (concat "#   " x))
                               (process-lines
                                "git" "log" "--pretty=format:%h  %cs  %s"
                                (format "-n %d" indiebrain-vc-git-log-edit-show-commit-count))
                               "\n"))))
    (unless no-headers
      (save-excursion
        (rfc822-goto-eoh)
    ...
    (goto-char (point-max))
    (insert "\n")
    (insert (indiebrain-vc--format-git-comment branch remote files commits))
    (rfc822-goto-eoh)
    (when (looking-at "\n") (forward-char -1))))

;;;###autoload
(defun indiebrain-vc-git-log-edit-previous-comment (arg)
...
With a numeric prefix ARG, go back ARG comments."
  ...
  (let ((len (ring-length log-edit-comment-ring)))
    (if (<= len 0)
      ...
      (indiebrain-vc-git-log-edit-comment t)
      (save-excursion
        (goto-char (point-min))
        (search-forward "# ---")
        (forward-line -1)
...
;;;###autoload
(defun indiebrain-vc-git-log-edit-next-comment (arg)
  "Cycle forwards through comment history.
With a numeric prefix ARG, go forward ARG comments."
  (interactive "*p")
  (indiebrain-vc-git-log-edit-previous-comment (- arg)))

(defvar indiebrain-vc--log-edit-comment-hist '()
  "History of inputs for `indiebrain-vc-git-log-edit-complete-comment'.")

(defun indiebrain-vc--log-edit-complete-prompt (comments)
  "Select entry from COMMENTS."
  (completing-read
   "Select comment: "
   comments nil t nil 'indiebrain-vc--log-edit-comment-hist))

;;;###autoload
(defun indiebrain-vc-git-log-edit-complete-comment ()
  ...
  (let* ((newline (propertize "^J" 'face 'escape-glyph))
         ...
         (completions
          (mapcar (lambda (s)
                    (string-replace "\n" newline s))
                  ring))
         (selection (indiebrain-vc--log-edit-complete-prompt completions))
         (comment (string-replace newline "\n" selection)))
    (add-to-history 'indiebrain-vc--log-edit-comment-hist comment)
    (delete-region (point-min) (point-max))
    (insert comment)
    (indiebrain-vc-git-log-edit-comment t)
    (save-excursion
...
(defun indiebrain-vc-git-log-remove-comment ()
...
;;;###autoload
(defun indiebrain-vc-git-log-edit-toggle-amend ()
...
(defun indiebrain-vc--buffer-string-omit-comment ()
...
(defun indiebrain-vc-git-log-edit-remember-comment (&optional comment)
...
commit message."
  (let ((commit (or comment (gensym))))
    (setq commit (indiebrain-vc--buffer-string-omit-comment))
...
(defvar indiebrain-vc--current-window-configuration nil
  "Current window configuration for use with Log Edit.")

(defvar indiebrain-vc--current-window-configuration-point nil
  "Point in current window configuration for use with Log Edit.")

(defun indiebrain-vc--store-window-configuration ()
  "Store window configuration before calling `vc-start-logentry'.
This should be called via `indiebrain-vc-git-pre-log-edit-hook'."
  (setq indiebrain-vc--current-window-configuration (current-window-configuration))
  (setq indiebrain-vc--current-window-configuration-point (point)))

(defvar indiebrain-vc-git-pre-log-edit-hook nil
  "Hook that runs right before `vc-start-logentry'.")

(defun indiebrain-vc-git-pre-log-edit (&rest _)
  "Run `indiebrain-vc-git-pre-log-edit-hook'.
To be used as advice before `vc-start-logentry'."
  (run-hooks 'indiebrain-vc-git-pre-log-edit-hook))

(defun indiebrain-vc--log-edit-restore-window-configuration ()
  "Set window configuration to the pre Log Edit state."
  (when indiebrain-vc--current-window-configuration
    (set-window-configuration indiebrain-vc--current-window-configuration))
  (when indiebrain-vc--current-window-configuration-point
    (goto-char indiebrain-vc--current-window-configuration-point)))

(defun indiebrain-vc--log-edit-diff-window-configuration ()
...
(defun indiebrain-vc--kill-log-edit ()
  "Local hook to restore windows when Log Edit buffer is killed."
  (when (or (derived-mode-p 'log-edit-mode)
            (derived-mode-p 'diff-mode))
    (add-hook 'kill-buffer-hook #'indiebrain-vc--log-edit-restore-window-configuration 0 t)))

(defvar indiebrain-vc-git-log-edit-done-hook nil
  "Hook that runs after `indiebrain-vc-git-log-edit-done'.")

;; FIXME: Why does `indiebrain-vc-git-log-remove-comment' not work when added
;; to `log-edit-done-hook'?
;;;###autoload
(defun indiebrain-vc-git-log-edit-done ()
  "Remove Git Log Edit comments and commit change set.
This is a thin wrapper around `log-edit-done', which first calls
`indiebrain-vc-git-log-remove-comment'."
  (interactive)
  (indiebrain-vc-git-log-remove-comment)
  (log-edit-done)
  (run-hooks 'indiebrain-vc-git-log-edit-done-hook))

(defface indiebrain-vc-git-log-edit-file-name
...
(defface indiebrain-vc-git-log-edit-local-branch-name
  '((default :inherit font-lock-comment-face)
    (((class color) (min-colors 88) (background light))
     :foreground "#0031a9")
    (((class color) (min-colors 88) (background dark))
...
(defface indiebrain-vc-git-log-edit-remote-branch-name
  '((default :inherit font-lock-comment-face)
    (((class color) (min-colors 88) (background light))
     :foreground "#55348e")
    (((class color) (min-colors 88) (background dark))
...
(defconst indiebrain-vc-git-log-edit-font-lock
  '(("^#.*"
     (0 'font-lock-comment-face))
    ("^#.*`\\(.+?\\)'.*`\\(.+?\\)'"
     (1 'indiebrain-vc-git-log-edit-local-branch-name t)
     (2 'indiebrain-vc-git-log-edit-remote-branch-name t))
    ("^#[\s\t][\s\t]+\\(.+\\)"
     (1 'indiebrain-vc-git-log-edit-file-name t)))
  "Fontification rules for Log Edit buffers.")

(defun indiebrain-vc-git-log-edit-extra-keywords ()
  "Apply `indiebrain-vc-git-log-edit-font-lock' to Log Edit buffers."
  (font-lock-flush (point-min) (point-max))
  (font-lock-add-keywords nil indiebrain-vc-git-log-edit-font-lock nil))
...
;;;###autoload
(define-minor-mode indiebrain-vc-git-setup-mode
...
`indiebrain-vc-git-log-edit-show-commits' is non-nil, also include a commit
log.  The number of commits in that log is controlled by
`indiebrain-vc-git-log-edit-show-commit-count'.

For Log Edit buffers, bind C-c C-c to `indiebrain-vc-git-log-edit-done' which
is designed to remove the comment block before checking in the changes.
Rebind other keys in the same vein.  `indiebrain-vc-git-log-edit-done' calls
the normal hook `indiebrain-vc-git-log-edit-done-hook' which is used to
...
inside the comment block."
  :init-value nil
  :global t
  (if indiebrain-vc-git-setup-mode
      (progn
        ;; Log view expanded commits
        (advice-add #'vc-git-log-view-mode :after #'indiebrain-vc-git-log-view-add-hook)
        (add-hook 'indiebrain-vc-git-log-view-mode-hook #'indiebrain-vc-git-expand-function)
        ;; Append comment block in Log edit showing branch and files.
        ;; This means that we no longer need the files' window to pop up
        ;; automatically
        (add-hook 'log-edit-hook #'indiebrain-vc-git-log-edit-comment)
        (remove-hook 'log-edit-hook #'log-edit-show-files)
        ;; Window configuration with just the commit and the diff
        ;; (restores previous state after finalising or aborting the
        ;; commit).
        (advice-add #'vc-start-logentry :before #'indiebrain-vc-git-pre-log-edit)
        (add-hook 'indiebrain-vc-git-pre-log-edit-hook #'indiebrain-vc--store-window-configuration)
        (advice-add #'log-edit-remember-comment :around #'indiebrain-vc-git-log-edit-remember-comment)
        (let ((map vc-git-log-edit-mode-map))
          (define-key map (kbd "C-c C-c") #'indiebrain-vc-git-log-edit-done)
          (define-key map (kbd "C-c C-e") #'indiebrain-vc-git-log-edit-toggle-amend)
        ...
        (add-hook 'log-edit-mode-hook #'indiebrain-vc--kill-log-edit)
        (add-hook 'indiebrain-vc-git-log-edit-done-hook #'indiebrain-vc--log-edit-restore-window-configuration)
        (add-hook 'log-edit-hook #'indiebrain-vc--log-edit-diff-window-configuration)
        ;; Extra font lock rules for Log Edit comment block
        (add-hook 'log-edit-hook #'indiebrain-vc-git-log-edit-extra-keywords))
    (advice-remove #'vc-git-log-view-mode #'indiebrain-vc-git-log-view-add-hook)
    (remove-hook 'indiebrain-vc-git-log-view-mode-hook #'indiebrain-vc-git-expand-function)
    (remove-hook 'log-edit-hook #'indiebrain-vc-git-log-edit-comment)
    (add-hook 'log-edit-hook #'log-edit-show-files)
    (advice-remove #'vc-start-logentry #'indiebrain-vc-git-pre-log-edit)
    (remove-hook 'indiebrain-vc-git-pre-log-edit-hook #'indiebrain-vc--store-window-configuration)
    (advice-remove #'log-edit-remember-comment #'indiebrain-vc-git-log-edit-remember-comment)
    (let ((map vc-git-log-edit-mode-map))
    ...
    (remove-hook 'log-edit-mode-hook #'indiebrain-vc--kill-log-edit)
    (remove-hook 'indiebrain-vc-git-log-edit-done-hook #'indiebrain-vc--log-edit-restore-window-configuration)
    (remove-hook 'log-edit-hook #'indiebrain-vc--log-edit-diff-window-configuration)
    (remove-hook 'log-edit-hook #'indiebrain-vc-git-log-edit-extra-keywords)))

(provide 'indiebrain-vc)
;;; indiebrain-vc.el ends here
