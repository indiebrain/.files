...
**** Departure
:PROPERTIES:
:CUSTOM_ID: h:9fb8284d-b1dc-4ce1-8f88-bd9531652fe1
:END:
...
** Gnome terminal
:PROPERTIES:
:header-args: :mkdirp yes :tangle (when (eq system-type 'gnu/linux) "./gnome-terminal/.gnome-terminal-profiles.dconf")
...
#+begin_src sh :tangle (when (eq system-type 'darwin) "./scripts/bin/initial-host-setup")
packages="aspell automake autoconf bash bash-completion@2 curl emacs git libtool ncurses openssl readline the_silver_searcher unzip"
#+end_src
...
packages="aspell automake autoconf bash curl emacs git libtool silversearcher-ag unzip"
#+end_src
...
configuration of them - setting up custom terminal profiles for
example. this next section allows for automated, post-installation
